...
function unpack(format, data) {
  ...
  while (formatPointer < format.length) {
    ...
    switch (instruction) {
      ...
      case 'Q': // unsigned long long (always 64 bit, machine byte order)
        if (quantifier === '*') {
          quantifier = (data.length - dataPointer) / 4;
        } else {
          quantifier = parseInt(quantifier, 10);
        ...
        currentData = data.substr(dataPointer, quantifier * 8);
        dataPointer += quantifier * 8;

        for (i = 0; i < currentData.length; i += 8) {
          currentResult =
                ((currentData.charCodeAt(i + 7) & 0xFF) << 80) +
                ((currentData.charCodeAt(i + 6) & 0xFF) << 64) +
                ((currentData.charCodeAt(i + 5) & 0xFF) << 48) +
                ((currentData.charCodeAt(i + 4) & 0xFF) << 32) +
                ((currentData.charCodeAt(i + 3) & 0xFF) << 24) +
                ((currentData.charCodeAt(i + 2) & 0xFF) << 16) +
                ((currentData.charCodeAt(i + 1) & 0xFF) << 8) +
                ((currentData.charCodeAt(i) & 0xFF));
      ...
      case 'N': // unsigned long (always 32 bit, little endian byte order)
        ...
        case 'd': //double
            ...
            for (i = 0; i < currentData.length; i += dataByteLength) {
              let dat = currentData.substr(i, dataByteLength);

              bytes = [];
              for (j = dat.length - 1; j >= 0; --j) {
                bytes.push(dat.charCodeAt(j));
              }
