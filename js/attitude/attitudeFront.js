...
export class AttitudeFront extends AttitudeBase {
    ...
    init(roll, yaw, pitch, dir) {
        ...
        this.cameraV = new Vector3(0,0,this.dist);
        ...
        for (let i = 1; i < 12; i++)    {
            l15[i] = new Linee("gray", 
                            new Vector3( -3 * this.dist * Math.sin(15*i/180*Math.PI), -3 * this.dist * Math.cos(15*i/180*Math.PI), -refd), 
                            new Vector3(3 * this.dist * Math.sin(15*i/180*Math.PI), 3 * this.dist * Math.cos(15*i/180*Math.PI), -refd),
                            0.05);
        ...
        disc.rotation.x = MathUtils.degToRad(0);
        disc.position.y = 0;
        disc.position.z = -refd+1;
        this.scene.add(disc);

        // attach Renederer to the page
        this.dom = this.renderer.domElement;
        document.getElementById("attitudeFront").appendChild(this.dom);
    ...
    loadModel(roll, yaw, pitch, dir) {
        ...
        this.oloader.load(
            ...
            function ( object ) {
                attitudeFront.setModel( object, roll, yaw, pitch, dir); 
            },
...
// main plotter object
window.attitudeFront = new AttitudeFront();
