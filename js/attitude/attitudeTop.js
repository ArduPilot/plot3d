...
/* global fpenv, THREE, Three, ZOOMSMIN, attitudeTop */
...
export class AttitudeTop extends AttitudeBase {    
    ...
    init(roll, yaw, pitch, dir) {
        ...
        // line along the box
        let line1 = new Linee("red", new Vector3(-3*this.dist,0,0), new Vector3(3*this.dist,0,0) , 0.05);
        this.scene.add( line1 );
        // line along the box
        let line2 = new Linee("navy", new Vector3(0,0,-3*this.dist), new Vector3(0,0,3*this.dist) , 0.05);
        this.scene.add( line2 );
        
        // 15 deg lines around the model
        let refd = this.dist*2-1;
        let l15 = [];
        for (let i = 1; i < 12; i++)    {
            l15[i] = new Linee("gray", 
                            new Vector3( -3 * this.dist * Math.sin(15*i/180*Math.PI), -refd, -3 * this.dist * Math.cos(15*i/180*Math.PI)), 
                            new Vector3(3 * this.dist * Math.sin(15*i/180*Math.PI), -refd, 3 * this.dist * Math.cos(15*i/180*Math.PI)),
                            0.05);
            this.scene.add(l15[i]);
        }
        
        // central disk covring the center of the 15 deg lines
        let radius = 9;
        ...
        disc.position.y = -refd+1;
        this.scene.add(disc);

        // attach Renederer to the page
        this.dom = this.renderer.domElement;
        document.getElementById("attitudeTop").appendChild(this.dom);
    ...
    loadModel(roll, yaw, pitch, dir) {
        ...
        this.oloader.load(
            ...
            function ( object ) {
                attitudeTop.setModel( object, roll, yaw, pitch, dir);
            },
...
// main plotter object
window.attitudeTop = new AttitudeTop();